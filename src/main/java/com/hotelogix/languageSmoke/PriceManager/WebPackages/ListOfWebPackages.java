package com.hotelogix.languageSmoke.PriceManager.WebPackages;
...
public class ListOfWebPackages {

	
	public String fn_getMessageText() throws Exception{
		try{
		String text=GenericMethods.GI().fn_GetText(GenericMethods.GI().getWebElement("A_ListOfWebPackages_Message_Text"));
		return text;
		}catch(Exception e){
			throw e;
		}
	...
	public void fn_verifyPkgStatus(String PkgName,String status) throws Exception{
		try{
		int count=GenericMethods.GI().fn_GetListWebElement("A_ListOfWebPackages_TrCount").size();
		for(int i=2;i<=count-2;i++){
            ...
            if(data.contains(PkgName)){
            	...
            	String activation=GenericMethods.GI().driver.findElement(By.xpath("//table[@class='list_viewnew']/tbody//tr["+i+"]//td[9]/img")).getAttribute("src");
	...
	public void fn_confgrAttachedPkg(String PkgName) throws Exception{
        ...
        for(int i=2;i<=count-2;i++){
        	String data=GenericMethods.GI().driver.findElement(By.xpath("//table[@class='list_viewnew']/tbody//tr["+i+"]//td[3]")).getText();
        	if(data.contains(PkgName)){
        		GenericMethods.GI().driver.findElement(By.xpath("//table[@class='list_viewnew']//tr["+i+"]//td[8]/a")).click();
        		break;
        	}       	
        }
		}catch(Exception e){
	...
	public String fn_getActivationDateLink(String PkgName) throws Exception{
		try{
	        ...
	        for(int i=2;i<=count-2;i++){
	        	String data=GenericMethods.GI().driver.findElement(By.xpath("//table[@class='list_viewnew']/tbody//tr["+i+"]//td[3]")).getText();
	        	if(data.contains(PkgName)){
	        		str=GenericMethods.GI().driver.findElement(By.xpath("//table[@class='list_viewnew']//tr["+i+"]//td[7]/a")).getText();
	        		break;
	        	}       	
	        }
	        return str;
	...
	public String fn_getActivatedForeverText(String PkgName) throws Exception{
		try{
	        ...
	        for(int i=2;i<=count-2;i++){
	        	String data=GenericMethods.GI().driver.findElement(By.xpath("//table[@class='list_viewnew']/tbody//tr["+i+"]//td[3]")).getText();
	        	if(data.contains(PkgName)){
	        		str=GenericMethods.GI().driver.findElement(By.xpath("//table[@class='list_viewnew']//tr["+i+"]//td[7]")).getText();
	        		break;
	        	}       	
	        }
	        return str;
